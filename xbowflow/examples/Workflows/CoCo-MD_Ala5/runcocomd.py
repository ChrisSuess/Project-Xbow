...
def runcocomd(client, args):
    ...
    inpcrds = client.upload(args['startcrds'])
    ...
    for cycle_number in range(1, ncycles + 1):
        ...
        for i in range(len(tnew)):
            with tempfile.NamedTemporaryFile(suffix='.rst7') as f:
                tnew[i].save(f.name)
                xrefs.append(client.upload(f.name))
        # Run tmd step 1
        restarts, mdcrds = client.map(pmemd1, inpcrds, xrefs)
        # Run tmd step 2
        restarts, mdcrds = client.map(pmemd2, restarts, xrefs)
        # Run production MD
        restarts, mdcrds = client.map(pmemd3, restarts, xrefs)
        # Add the new trajectory data to the collection
        new_trajs = client.map(maketraj, mdcrds)
        for t in new_trajs:
            t_all += t.result()
